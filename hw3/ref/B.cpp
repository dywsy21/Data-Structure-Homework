...
{
	if(p->flag == 0) return ;
	for(int i=1;i<=p->last/2;i++) swap(p->a[i], p->a[p->last-i+1]);
	p->flag = 0;
}
void check(node *p)
{
	if(p->last < 2*len) return ;
	push(p);
	node *newnode = &pool[++cnt];
	newnode->next = p->next; newnode->pre = p;
	if(p->next) p->next->pre = newnode;
	p->next = newnode;
...
bool check2(node *p)
{
	...
	if(p->last + p->next->last < 2*len) return true;
	return false;
}
...
//	print();
	push(p1); push(p2);
	for(int i=1;i<=p2->last;i++) p1->pb(p2->a[i]);
	if(p2->next) 
	{
		p2->next->pre = p1;
	}
...
//		scanf("%d%d",&l,&r);
		l = read(); r = read();
		p1 = head; suml = p1->last;
		while(suml < l)
		{
			p1 = p1->next;
			suml += p1->last;
		}
		p2 = head; sumr = p2->last;
		while(sumr < r)
		{
			p2 = p2->next;
			sumr += p2->last;
